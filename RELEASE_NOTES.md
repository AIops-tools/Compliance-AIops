# Release notes — compliance-aiops 0.6.0
...
- **`bundle generate` / `bundle export` now audit.** They called the ops layer directly, so the tool that seals evidence from every *other* tool's audit DB left its own primary write unrecorded. Both now route through the governed twin; a regression test asserts the CLI seal lands an audit row.

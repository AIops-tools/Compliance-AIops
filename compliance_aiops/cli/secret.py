"""``compliance-aiops secret`` — manage the encrypted credential store.

Secrets (Compliance API keys) are stored in ``~/.compliance-aiops/secrets.enc``
...
from compliance_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
from compliance_aiops.secretstore import (
    SECRETS_FILE,
    SecretStore,
    check_permissions,
    migrate_legacy_env,
...
def secret_list() -> None:
    ...
    if not names:
        console.print(
            "[yellow]No secrets stored yet. Add one: compliance-aiops secret set <name>[/]"
        )
        return
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update COMPLIANCE_AIOPS_MASTER_PASSWORD.[/]")

...
2. **Runaway breaker (on by default).** The same ``(tool, params)`` called more
   than ``COMPLIANCE_RUNAWAY_MAX`` times within ``COMPLIANCE_RUNAWAY_WINDOW_SEC`` trips a
...
from compliance_aiops.governance.policy import PolicyResult
...
class BudgetExceeded(Exception):
    ...
    Subclasses nothing Compliance-specific; the decorator wraps it into a

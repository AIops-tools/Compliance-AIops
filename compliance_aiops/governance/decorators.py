"""The ``@governed_tool`` decorator — mandatory wrapper for all Compliance MCP tool functions.

Responsibilities:
  1. Pre-check: evaluate policy rules (deny, maintenance window)
  2. Execute: run the actual tool function
  3. Post-log: write audit record to ``~/.compliance-aiops/audit.db``
...
from compliance_aiops.governance.audit import detect_agent, get_engine
from compliance_aiops.governance.budget import BudgetExceeded, get_budget
from compliance_aiops.governance.outcome import (
    clear_prior_state,
    is_unknown,
    take_prior_state,
)
from compliance_aiops.governance.patterns import PatternMatch, get_pattern_engine
from compliance_aiops.governance.policy import PolicyResult, get_policy_engine
from compliance_aiops.governance.readonly import is_read_only, read_only_denial
from compliance_aiops.governance.sanitize import sanitize

_log = logging.getLogger("compliance-aiops.decorators")
...
def governed_tool(
    ...
    *,
    ...
    """Decorator for all Compliance MCP tool functions.
    ...
    Args:
        ...
        undo: Optional callable ``(params, result) -> dict | None`` returning an
            ...
            successful call the inverse is recorded to ~/.compliance-aiops/undo.db and the
...
class _CallState:
    ...
    def __init__(
        ...
        self.rationale = os.environ.get("COMPLIANCE_AUDIT_RATIONALE", "")
        self.approved_by = os.environ.get("COMPLIANCE_AUDIT_APPROVED_BY", "")
        self.risk_tier = ""
...
def _pre_check(state: _CallState) -> None:
    ...
    # review) are denied when none was recorded (COMPLIANCE_AUDIT_APPROVED_BY).
    ...
    if tier.requires_approver and not state.approved_by and not state.dry_run:
        reason = (
            ...
            f"Set COMPLIANCE_AUDIT_APPROVED_BY to the authorizing human (and "
            f"COMPLIANCE_AUDIT_RATIONALE to why) before retrying."
        )
        if tier.reason:
...
def _record_undo(state: _CallState, result: Any) -> None:
    ...
    * **Undetermined** — the response was lost (see
      :mod:`compliance_aiops.governance.outcome`). The change may be live, so an
    ...
    if not descriptor:
        return
    try:
        from compliance_aiops.governance.undo import get_undo_store
...
def _finalize(state: _CallState) -> None:
    ...
    # cancellation (cancelling mid-flight Compliance calls is worse).
...
def _infer_skill(func: Any) -> str:
    ...
    ``compliance_aiops.ops.jobs`` → ``compliance-aiops``
    ``mcp_server.server`` → ``compliance-aiops`` (the only consumer here).
    """
    module = getattr(func, "__module__", "") or ""
    if module.startswith("compliance_aiops") or module.startswith("mcp_server"):
        return "compliance-aiops"
    return "unknown"

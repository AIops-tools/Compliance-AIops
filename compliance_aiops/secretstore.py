"""Encrypted secret storage for compliance-aiops.

Secrets (Compliance API keys) are NEVER written to disk in plaintext. They
live in ``~/.compliance-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``COMPLIANCE_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
APP_NAME = "compliance-aiops"
CONFIG_DIR = Path.home() / ".compliance-aiops"
MASTER_PASSWORD_ENV = "COMPLIANCE_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``COMPLIANCE_<TARGET>_PASSWORD``; ``prefix='COMPLIANCE_'`` and

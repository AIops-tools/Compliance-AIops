...
def test_cli_undo_apply_dry_run_of_an_unknown_id_is_refused_nonzero(gov_home):
    ...
    ``undo apply --dry-run`` was already routed through the governed twin, so
    the guard ran — but its refusal was dropped on the floor: the banner
    rendered ``inverse: ?`` and exited 0. A green preview for an undo that can
    never run is the failure mode a weak model reads as transient and retries.
    """
    from typer.testing import CliRunner

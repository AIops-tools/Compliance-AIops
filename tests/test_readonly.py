...
from compliance_aiops.governance import READ_ONLY_ENV, is_read_only
...
def test_env_var_is_this_tools_prefix():
    """Brand isolation: compliance-aiops must not read another tool's switch."""
    assert READ_ONLY_ENV == "COMPLIANCE_READ_ONLY"
...
def test_governed_write_is_refused_in_read_only(read_only):
    """A write is refused before it can touch the filesystem — from any caller."""
    calls = []

    @governed_tool(risk_level="high")
    def overwrite_a_bundle(path: str = "bundle.json") -> dict:
        calls.append(path)
        return {"written": True}
...
def test_governed_read_still_works_in_read_only(read_only):
    ...
    def query_events(selector: str = "change") -> list:
        return [{"tool": "vm_delete"}]

    assert query_events() == [{"tool": "vm_delete"}]


@pytest.mark.unit
def test_medium_risk_write_is_also_refused(read_only):
    """Only 'low' is a read — medium (e.g. export_bundle) must be refused too."""

    @governed_tool(risk_level="medium")
    def export_a_bundle(path: str = "bundle.json") -> dict:
...
def test_apply_read_only_unregisters_write_tools(monkeypatch):
    ...
    try:
        ...
        # Everything that puts bytes on disk goes, including bundle generation:
        # read-only means this tool produces no artifacts, only answers.
        assert "generate_evidence_bundle" in dropped and "export_bundle" in dropped
        assert "sign_bundle" in dropped
        assert "query_audit_events" not in dropped and "verify_bundle" not in dropped
        ...
        assert "query_audit_events" in remaining, "reads must still be exposed"
    finally:
        registry.clear()

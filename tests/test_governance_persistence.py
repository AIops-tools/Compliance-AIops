...
throwaway home (``COMPLIANCE_AIOPS_HOME``) and assert that the rows compliance
...
import compliance_aiops.governance.policy as policy_mod
import compliance_aiops.governance.undo as undo_mod
from compliance_aiops.governance import PolicyDenied, governed_tool
...
def gov_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("COMPLIANCE_AUDIT_APPROVED_BY", raising=False)
    monkeypatch.delenv("COMPLIANCE_POLICY_DISABLED", raising=False)
    _reset_singletons()
    yield tmp_path
    _reset_singletons()
...
def test_medium_write_persists_audit_and_undo_rows(gov_home):
    result = _rename_widget(name="w1", target="prod-evidence")
...
def test_high_risk_denied_without_approver_and_denial_is_audited(gov_home):
    ...
    with pytest.raises(PolicyDenied, match="requires 'dual' approval"):
        _drop_widget(name="w2", target="prod-evidence")
...
def test_high_risk_allowed_with_named_approver(gov_home, monkeypatch):
    monkeypatch.setenv("COMPLIANCE_AUDIT_APPROVED_BY", "auditor-alice")
    result = _drop_widget(name="w3", target="prod-evidence")
    assert result["status"] == "dropped"

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert len(audit) == 1
    assert audit[0]["status"] == "ok"
    assert audit[0]["approved_by"] == "auditor-alice"
...
def test_operator_rules_file_restores_tier_none_for_high_risk(gov_home):
    ...
    _reset_singletons()
    result = _drop_widget(name="w4", target="prod-evidence")
    assert result["status"] == "dropped"


@pytest.mark.unit
def test_real_write_tool_persists_audit_row(gov_home, monkeypatch):
    """End-to-end through a REAL product write tool: generate_evidence_bundle
    must write the bundle artifact AND land its own audit row on disk.

    (No governed tool in this package declares an undo callback — bundle
    generation only creates new local artifacts — so unlike the platform
    tools this asserts audit persistence, not an undo row.)
    """
    ...
    source_db = gov_home / "source.db"
    AuditEngine(str(source_db))  # real audit_log schema, zero rows
    reader = AuditReader([AuditSource(name="testsrc", path=source_db)])
    cfg = AppConfig(
        sources=(AuditSource(name="testsrc", path=source_db),),
        organization="Acme",
        bundle_dir=gov_home / "bundles",
    )
    _reset_singletons()  # the AuditEngine above must not pin the singleton

    from mcp_server.tools import bundle as gov

    monkeypatch.setattr(gov, "_get_reader", lambda: reader)
    monkeypatch.setattr(gov, "_get_config", lambda: cfg)
    result = gov.generate_evidence_bundle(framework="soc2")
    assert "error" not in result
    assert result["chainHead"]
    assert (gov_home / "bundles").exists()
